...
- Supports direct Claude Code, Codex, and GitHub Copilot execution, locally-hosted Ollama models (routed through the Codex harness), Maestro delegation, and a deterministic scripted stub through one non-raising adapter contract; Windows deadlines terminate the complete npm CLI process tree rather than only its wrapper.
...
The orchestrator now **collects 221 tests across 10 test files**, including Copilot parsing/classification and target-repository artifact coverage. `compileall` passes. A fresh full pytest attempt in this Windows checkout is currently blocked by `[WinError 5]` ACL failures while pytest handles temporary directories, so do not treat the collection count as a fresh complete-suite pass. The suite uses the first-class stub transport, so graph, state, worktree, merge, rework, replan, and failure behavior can be exercised deterministically without paid agent calls.
...
| Track | Purpose | State |
|---|---|---|
| `orchestrator/` | Production six-stage workflow and its tests. | Implemented; 221 tests collected, with static validation passing. |
...
Open production findings are recorded in `docs/Bugs.md` #26–#28, #32–#33, #35, #38, and #40–#44. Bug #36's project-scope artifact gap is fixed: shared memory now lives under the target repository while audit records remain per-run. Bug #42 still depends on Copilot/GitHub service health, #43 tracks subscription-gated Ollama Cloud roster entries, and #44 records the planned direct local OpenAI-compatible adapter. Bug #34—the Windows npm-shim timeout—remains fixed across all three vendor adapters; #37 and #39 were fixed by correcting the model menu and removing the incompatible Claude Ollama harness path.
