...
The last complete repository-wide baseline remains the September 8 run: **581 collected, 547 passing, 34 failing** — one pre-existing unrelated Copilot assertion plus 33 open adversarial `mem_manager` safety-contract findings (`docs/Bugs.md` #63). September 9's two memory commits added session provenance/salience and durable file-specific titles. During this documentation update, 23 targeted regressions covering marker placement, legacy/sessionless entries, peer isolation, metadata round-tripping, repeated compaction, chronological naming, and unconfigured-file compatibility passed; this focused result is not presented as a new full-suite baseline. The session field now survives Bug #63's metadata round-trip test, while the other metadata and adversarial families remain open. Coding-subagent helper writes still lack a truthful backend session (`docs/Bugs.md` #64).
...
| Track | Purpose | State |
|---|---|---|
| `orchestrator/` | Production six-stage workflow and its tests. | Implemented; 297 tests passing (1 pre-existing timeout). |
...
September 9 partially closed #63's metadata-loss family by preserving backend-session identity, opened #64 for still-sessionless out-of-process coding-agent writes, and closed #65 as not reproduced after both named compacted files were found fully renumbered on disk. The active mechanical roster now uses Gemini `gemini-3.1-flash-lite` for requirements, wave orchestration, and merge; planner, reviewer, and supervisor use Codex CLI default.
