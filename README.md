...
- Supports direct Claude Code, Codex, and GitHub Copilot execution, locally-hosted Ollama models (routed through the Codex harness), configurable OpenAI-compatible local servers through the separate `direct:local_llm` bridge, optional Maestro delegation, and a deterministic scripted stub through one non-raising adapter contract; Windows deadlines terminate the complete npm CLI process tree rather than only its wrapper.
...
Open production findings are recorded in `docs/Bugs.md` #26-#28, #32-#33, #35, #38, #40-#43, and #47. Bug #36's project-scope artifact gap is fixed: shared memory now lives under the target repository while audit records remain per-run. Bug #42 still depends on Copilot/GitHub service health, #43 tracks subscription-gated Ollama Cloud roster entries, and #44 is now closed by the shipped `direct:local_llm` backend. Bug #34 - the Windows npm-shim timeout - remains fixed across all three vendor adapters; #37, #39, and #46 were fixed by correcting the model menu, removing the incompatible Claude Ollama harness path, and matching the live Copilot event shape.
