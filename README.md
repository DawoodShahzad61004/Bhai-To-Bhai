...
The suite now **collects 235 tests across 12 test files**. The last documented full green run in repository history remains the Aug 18 local-LLM validation session at **231 passing tests**, run through the repository virtual environment with an isolated pytest temp directory. Since then, focused regressions were added for Local LLM memory isolation and bridge filtering, but a fresh Aug 21 full pytest rerun from this checkout still reproduces the long-standing Windows temp-directory permission cleanup failure at session finish. The current turn therefore claims the 235-test collection count, not a new full green suite. The suite uses the first-class stub transport, so graph, state, worktree, merge, rework, replan, and failure behavior can be exercised deterministically without paid agent calls.
