...
| Module | Responsibility |
|---|---|
| `orchestrator/mem_manager/importance.py` | Parses episodic markdown (`## DATE TIME SEP TAG` headers, plus a `USER_CHOICES_HEADER_PATTERN` fallback for `user_choices.md`'s run-id-first shape) and computes a five-factor composite importance score per record — recency, frequency, surprise, entity salience, outcome. |
| `orchestrator/mem_manager/memory.py` / `consolidate.py` | Converts scored episodic records into `DurableMemory` objects, applies passive decay based on age and last-access time, and prunes the bottom `PRUNE_BOTTOM_PERCENT` of low-importance entries once the corpus exceeds `MIN_PRUNE_BUDGET`. |
...
| Tests | pytest, 547 passing (581 collected across the full repository, including `orchestrator/tests/memory_scenarios/`) |
