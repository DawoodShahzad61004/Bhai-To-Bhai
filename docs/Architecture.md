...
Known open findings are tracked in `docs/Bugs.md` #26–#28 (incomplete merge-context propagation, duplicate requirements routing logic, and successful-run worktree cleanup), #32–#33 (unborn-repository worktree setup and a browser-rendered HTML defect), #35–#36 (wave-wide rework discarding completed integrations, and run-scoped artifacts remaining non-project-scoped), and #38 (static roster validation does not yet route around live quota exhaustion).
