...
As of `Decisions.md` ADR-049, `learnings.md` and `user_choices.md` also self-compact: every `append_learning()`/`append_user_choices()` call checks `should_auto_compact()` after its own write and, once the file both clears `config.MIN_PRUNE_BUDGET` and is a configured `config.COMPACT_ARTIFACT_FILES` target, runs `/compact` on it in place. A compacting write takes the identical sidecar-file lock every other writer to that path already takes, so it is mutually exclusive with a concurrent append rather than racing it; a concurrent *reader* (e.g. `peer_entries_since()`) takes no lock at all, since the compacted content is written to a temp file and swapped in with one atomic `os.replace()`, so a read either sees the file whole before compaction or whole after, never partial.
...
| `orchestrator/diagnostics.py` | Standalone, opt-in live-availability check for every active configured `(backend, model)` pair, deduplicated across `config.AGENTS`, the coding-agent fallback slots, and the small/medium/expert menus; probes each pair once through `adapters.run_agent()` with a minimal structured-output contract and bounded concurrency, and can gate `main.py` startup on the result (`config.ENABLE_AGENT_DIAGNOSTICS`, disabled by default; `Decisions.md` ADR-044). |
...
### Memory Consolidation (`mem_manager`) and the `/compact` Command

| Module | Responsibility |
|---|---|
| `orchestrator/mem_manager/importance.py` | Parses episodic markdown (`## DATE TIME SEP TAG` headers, plus a `USER_CHOICES_HEADER_PATTERN` fallback for `user_choices.md`'s run-id-first shape), extracts an optional first-body-line `<!-- session:<backend>:<session-id> -->` marker as metadata, and computes a five-factor composite importance score per record — recency, frequency, surprise, session salience, outcome. Session salience is normalized by how many records each writing session contributed; legacy/sessionless entries receive no lift rather than sharing a fabricated empty identity. |
| `orchestrator/mem_manager/memory.py` / `consolidate.py` | Converts scored episodic records into `DurableMemory` objects, carries the keeper's writing session through merge/consolidation, applies passive decay based on age and last-access time, and prunes the bottom `PRUNE_BOTTOM_PERCENT` of low-importance entries once the corpus exceeds `MIN_PRUNE_BUDGET`. |
| `orchestrator/mem_manager/services/` | Near-duplicate detection via complete-linkage clustering over embedding similarity (`MERGE_SIMILARITY_THRESHOLD`), deterministic-union merge with optional LLM-assisted merge and judge validation, embedding generation with timeout handling, and a FIFO rate-limited LLM call gate against an OpenAI-compatible endpoint. |
| `orchestrator/compact_command.py` | Entry point for `main.py --compact`: reads target files from `config.COMPACT_ARTIFACT_FILES`, runs the parse → score → dedup/merge → consolidate → prune pipeline on each file concurrently via `asyncio`, reconstructs episodic markdown (including retained session markers), and **overwrites each source file completely** rather than appending (`Decisions.md` ADR-045). Also exposes `should_auto_compact()`/`maybe_auto_compact()`, called from `artifacts.py` after every append (see below), and `_compact_and_swap()`, which computes the compacted content into a temp file and swaps it in with a single atomic `os.replace()` under the same writer lock every appender to that path already takes, so a concurrent reader is never exposed to a partial rewrite (`Decisions.md` ADR-049). `compact_markdown_file()` then assigns file-specific chronological titles after merge/prune: `Learning 1`, `Learning 2`, ... for `learnings.md`, and `User Choice 1`, `User Choice 2`, ... for `user_choices.md`; files absent from `config.DURABLE_MEMORY_TAG_PREFIXES` retain their source tags (ADR-048). Every surviving memory's `<!-- id: -->`, `<!-- provenance: -->`, `<!-- last_accessed_at: -->`, `<!-- merged_from: -->`, and `<!-- session: -->` metadata round-trips through the rewrite, so a memory compacted on one pass keeps a stable identity and its full metadata on the next (`Bugs.md` #63, closed 2026-09-10). A `NoOpEmbedder` fallback (dummy embeddings, `0.0` similarity) lets the pipeline complete with deterministic-only merge when `numpy`/`sentence-transformers`/`groq`/`openai`/etc. are unavailable, logging a `WARNING` and retrying with `use_llm=False` rather than failing. |

The pipeline is entirely independent of the six-stage production graph — `mem_manager` is imported directly into the orchestrator process (`from mem_manager.compact import compact_markdown_file`), not run as an isolated subprocess, and shares the project's single `.venv` (`Decisions.md` ADR-046). Orchestrator-owned writers derive `<backend>:<session_id>` with `artifacts.session_key()` and stamp learnings from requirements, planner, merger, reviewer, and supervisor; `peer_entries_since()` strips the marker before exposing a finding to another agent. Requirements also places the marker inside the `user_choices.md` block it writes. Coding-subagent `append-learning` and `run-shared` writes remain sessionless because those out-of-process helpers do not receive the dispatch `AgentResult` (`Bugs.md` #64). `config.COMPACT_ARTIFACT_FILES` currently targets the same `.bhai-artifacts/<target>/shared/` directory the production pipeline's `context.md`/`learnings.md`/`user_choices.md` already live in (ADR-038). As of `Decisions.md` ADR-049, `/compact` no longer runs only on an explicit `main.py --compact` invocation: `artifacts.append_learning()` and `append_user_choices()` each call `_maybe_trigger_auto_compact()` after their own write, which fires `maybe_auto_compact()` whenever `config.ENABLE_PRUNING` is true, the written path is one of `config.COMPACT_ARTIFACT_FILES`, and the file's size clears `config.MIN_PRUNE_BUDGET` — the same two switches `rerank_and_prune()` itself reads, so auto-compact cannot fire under conditions where pruning is disabled. `mem_manager/requirements.txt` pulls a materially heavier dependency set than the rest of the orchestrator (`numpy`, `torch`, `sentence-transformers`, `transformers`, `scikit-learn`, `openai`, `groq`, `langchain-openai`, `langchain-groq`, `chromadb`) — see Technology Stack below and ADR-046's Impact field for the open question this raises against root `requirements.txt`'s stated "no model client" design principle. The September 8 adversarial baseline (`Bugs.md` #63) is now **closed**: all 32 remaining safety-contract scenarios pass as of the September 10 session, alongside a new `orchestrator/tests/memory_scenarios/` quality-benchmark suite (`quality_manifest.json`, `quality_runner.py`, `quality_report.py`, and nine checked-in corpus scenarios `s1`–`s9`) that measures compaction usefulness directly as paired keyword-recall/tokens-freed figures rather than only pass/fail contracts (`Decisions.md` ADR-050).
...
| Memory consolidation (`mem_manager`, optional) | `numpy`, `torch` (CUDA build), `sentence-transformers`, `transformers`, `scikit-learn`, `openai`, `groq`, `langchain-openai`, `langchain-groq`, `chromadb` — a materially heavier, model-client-bearing dependency set than the rest of the orchestrator; degrades gracefully to deterministic-only merge (`NoOpEmbedder`) when absent |
| Tests | pytest; last full-repository baseline: **834 passing, 0 failing** (2026-09-10, up from 547 passing / 581 collected on 2026-09-08). Includes a 224-test quality-benchmark suite for `mem_manager` (`Decisions.md` ADR-050). |
...
Commit `d347746` ("Fix logging setup in tests, add auto-compaction tests, and update copilot adapter command assertions") added auto-compact: `artifacts._maybe_trigger_auto_compact()` fires after every `append_learning()`/`append_user_choices()` call, gated on the same `config.ENABLE_PRUNING`/`MIN_PRUNE_BUDGET` switches `rerank_and_prune()` already reads. A compacting write takes the same sidecar-file lock every writer to that path already takes (`compact_command._lock_target_for()` maps `user_choices.md` onto `learnings.md`'s lock); a concurrent read takes no lock at all, since `_compact_and_swap()` writes the compacted result to a temp file and swaps it in with one atomic `os.replace()` (`Decisions.md` ADR-049). The same commit closed all 32 remaining `test_safety_contracts.py` failures (`Bugs.md` #63, now **closed**): full metadata round-tripping (`<!-- id: -->`, `<!-- provenance: -->`, `<!-- last_accessed_at: -->`, `<!-- merged_from: -->`) through `_memories_to_markdown()`; a peer-state invalidation helper so a compacting rewrite does not leave a peer's `learnings.stamp`/cursor sidecars stale; an abort-rather-than-overwrite guard when parsing yields zero records from non-empty input; a negation-marker asymmetry guard so a statement and its direct negation never merge; a deferred, `try`/`except`-wrapped LLM client construction instead of a missing fallback path; an exact `"FAITHFUL"` judge-verdict match instead of a substring test; and `prune_fraction` validation into `[0, 1]`. Two unrelated test-hygiene defects surfaced by the same full-suite run were fixed alongside it: the `offline` fixture's `setup_logging` mock rejected the `app_name` keyword `compact_command.py` already passed (`Bugs.md` #66), and a Copilot adapter test still asserted a `--allow-all-tools` flag the adapter had already stopped emitting (`Bugs.md` #67). Full suite after this commit: 610 passing, 0 failing (up from 547 passing / 34 failing on 2026-09-08).

Commit `fd5fb05` ("Add comprehensive tests for quality metrics and loss detection") added a quality-benchmark harness under `orchestrator/tests/memory_scenarios/`: nine checked-in corpus scenarios (`s1_verbatim_duplicates.md` through `s9_auto_prefix.md`) plus a golden `*.keywords.json` per file, scoring compaction on paired type-level keyword recall and a pluggable, named tokens-freed counter — never one without the other, since either alone is gameable (doing nothing scores perfect recall; deleting everything scores strong savings). The LLM-merge-rewrite lane runs by default against deterministic synthetic stub rewriters (faithful, lossy, judge-rejected, fact-inventing) that prove the metric detects each failure mode, since the offline deterministic-union merge path is a verbatim join and therefore guarantees 1.0 recall by construction except where pruning or malformed input is involved; a real-provider run is a separate `MEM_QUALITY_LIVE=1`-gated script (`quality_live.py`), structurally unreachable from `pytest` itself because the suite's autouse `offline` fixture replaces the LLM-call constructor with a tripwire (`Decisions.md` ADR-050; `Research.md` topics 62-63). 224 new tests across five `test_quality_*.py` files brought the full suite to 834 passing, 0 failing.

Commit `fa1d295` ("fix: reconfigure stdout/stderr to UTF-8 for compact command compatibility") fixed a third occurrence of the Windows `cp1252`-console-codepage `UnicodeEncodeError` defect class first recorded as `Bugs.md` #11 and fixed a second time as #57: `compact_command()`'s own human-readable summary printed a ✗/✓ status marker that crashed under a real Windows console (masked under ordinary `pytest` runs, which capture stdout through a UTF-8-safe buffer). Fixed with the same `sys.stdout`/`sys.stderr.reconfigure(encoding="utf-8", errors="replace")` pattern already used in `artifacts.py`'s `_cli()` (`Bugs.md` #68).

---

### 2026-09-09 - Memory identity changed from prose entities to backend sessions; compacted files gained durable chronological titles

Commit `7436c8c` replaced regex-derived text entities with the agent backend session that wrote each memory. `artifacts.session_key()` forms `<backend>:<session_id>` without inventing an identity for missing ids; orchestrator-owned learning writers stamp `<!-- session:... -->` immediately below the episodic header, requirements carries the same marker into its user-choice block, and the parser removes it from content while retaining it as metadata. `DurableMemory.session`, merge/consolidation paths, `_memories_to_markdown()`, and compact-command result reporting preserve the identity through a destructive full-file rewrite. `peer_entries_since()` strips markers from findings shown to other agents. Importance now uses session-record frequency (`build_session_index()` / `f_session_salience()`) for the existing 0.15 factor instead of text-entity salience (ADR-047; `Research.md` topic 60). Coding-subagent helper writes remain sessionless (`Bugs.md` #64).

Commit `7c422c3` added `config.DURABLE_MEMORY_TAG_PREFIXES` and applied file-specific titles after merge/prune in `compact_markdown_file()`: `learnings.md` becomes `Learning 1`, `Learning 2`, ... and `user_choices.md` becomes `User Choice 1`, `User Choice 2`, ..., numbered by `created_at` even when the returned memory list is ranked differently. Unconfigured files keep their original tags. A follow-up check of the two named `.bhai-artifacts/.../shared/` files found all 54 on-disk headers conforming (28 user choices, 26 learnings); the reported old titles were not reproduced and no code changed (`Bugs.md` #65). Focused verification for the combined September 9 behavior: 23 targeted tests passed (ADR-048; `Research.md` topic 61).

---

### 2026-09-08 - A memory-consolidation subsystem (`mem_manager`) and `/compact` command added; a header-parsing bug and a log-naming gap fixed; dependency conflicts with root resolved; 283 adversarial tests find 33 open safety-contract gaps

Commit `03f90f7` ("Added embedding and LLM management services") added `orchestrator/mem_manager/`: episodic-markdown parsing and five-factor composite importance scoring in `importance.py`; durable-memory conversion, decay, and budget-gated pruning in `memory.py`/`consolidate.py`; complete-linkage near-duplicate clustering and deterministic-or-LLM-assisted merge with judge validation in `services/`; and `orchestrator/compact_command.py`, the entry point behind the new `main.py --compact` flag. The command's design went through three explicit user corrections in one session, recorded together as `Decisions.md` ADR-045: hardcoded direct module invocation instead of a formal Skill, artifact target paths moved into `config.COMPACT_ARTIFACT_FILES` instead of literals, and full-file rewrite of consolidated output instead of append (append would defeat the pipeline's own `PRUNE` stage). A `NoOpEmbedder` fallback keeps the pipeline running with deterministic-only merge when ML dependencies are absent.

Commit `50d58be` ("Add comprehensive tests for memory scenarios...") added 283 scenario-based tests under `orchestrator/tests/memory_scenarios/`, grounded in `mem_manager`'s own design documents: 250 passed cleanly across CRUD/parsing/scoring/decay/pruning, merge/embeddings/LLM-judge decisions, artifact/command integration, and sequential-and-concurrent writers (720 sequential operations across 12 runs); 33 of 34 adversarial safety-contract scenarios failed as ordinary assertions and were left open, itemized as `Bugs.md` #63 (`Research.md` topic 59). The same commit fixed a real parsing defect found while diagnosing a live compact run: `user_choices.md`'s run-id-first header shape did not match the standard episodic pattern and silently parsed to 0 records, closed with a new `USER_CHOICES_HEADER_PATTERN` fallback wired into `parse_episodic_md()` (`Bugs.md` #61).

Commit `9f0998e` ("Update logging setup and modify model configurations in compact command") fixed `compact_command.py` falling back to the generic `"orchestrator"` log app-name instead of a dedicated one (`Bugs.md` #62, matching `diagnostics.py`'s existing `app_name="diagnostics"` pattern), then resolved a real dependency conflict between root `requirements.txt` and `orchestrator/mem_manager/requirements.txt` using `uv pip compile` against root's pins as constraints rather than guessing: three genuine conflicts (`langgraph`, `python-dotenv`, `pytest`) were dropped from `mem_manager/requirements.txt` in favor of root's versions, two apparent-but-false ones (`langgraph-checkpoint`, `langchain-core`) were kept, and `torch==2.14.0+cu130` was installed separately against the PyTorch CUDA wheel index before the rest of the file from PyPI only, after a combined `--extra-index-url` install failed on uv's dependency-confusion guard (`Decisions.md` ADR-046). This closes the `NoOpEmbedder`/no-LLM fallback that had been silently degrading every `/compact` run's dedup quality up to this point.

`orchestrator/COMPACT_USAGE.md` and the repository-root `Ollama Coding Agent Working.md` (both temporary working files) were retired after folding `COMPACT_USAGE.md`'s still-relevant content into this document's new Memory Consolidation module section and confirming `Ollama Coding Agent Working.md`'s findings were already fully captured in `Bugs.md` #40-#41 and `Research.md` topics 32-33.

---

### 2026-08-31 - A standalone, opt-in configured-backend diagnostic added and given input/output logging; its first live run found a real local_llm defect

Commit `682f8ae` ("Diagnostic Module added") added `orchestrator/diagnostics.py`: a standalone module that collects every active configured `(backend, model)` pair from `config.AGENTS`, `CODING_AGENT_A`/`CODING_AGENT_B`, and the `SMALL_MODELS`/`MEDIUM_MODELS`/`EXPERT_MODELS` menus, deduplicates by `(backend, model)` keeping the smallest applicable deadline and every configuration source for reporting, and probes each unique pair exactly once through the existing `adapters.run_agent()` boundary with a minimal `{"diagnostic":"ok"}` structured-output contract, zero tools, and an isolated temp working directory. Probing runs with `ThreadPoolExecutor` bounded by new config value `AGENT_DIAGNOSTIC_MAX_PARALLEL=3`. The module is usable standalone (`python orchestrator/diagnostics.py`, `--json`) and, only when new config flag `ENABLE_AGENT_DIAGNOSTICS` (default `False`) is set, gates both new and resumed pipeline runs — aborting before any artifacts, checkpoints, or graph execution begin if any probe fails (`Decisions.md` ADR-044). Tests: `orchestrator/tests/test_diagnostics.py` (268 lines) plus `test_main.py` additions; suite reached 294 collected (293 passing, the one pre-existing Copilot failure).

The example configuration's first live run against all nine resulting targets found a real production defect on its very first execution: `direct:local_llm`/`QuantTrio/Qwen3.6-27B-AWQ` failed with a 422 context-overflow, because `orchestrator/adapters/local_llm_bridge.py` treated an empty `tools=()` tuple as "no tool filter" and forwarded all 227 Codex tool schemas (335 KB, 97,379 tokens) on every call — including this diagnostic's own deliberately zero-tool probe. Fixed the same session: the bridge now forwards zero tools for `tools=()` and recognizes this server's specific 422 shape as a classified error (`Bugs.md` #58; `Research.md` topic 58).

Commit `87a1017` ("Logging added in the Diagnostic Module") added input/output/verdict logging to `diagnostics.py` at DEBUG level, matching the `_debug_block()` head/tail-truncation convention the five adapter modules already use. A `.tag` property on `DiagnosticTarget` correlates one target's dispatch, input, output, and verdict lines across an interleaved multi-probe log; the standalone CLI now calls `setup_logging(app_name="diagnostics")` and prints the resulting log path. Fixing this exposed two further defects, both closed the same day: `main.py`'s pre-run gate ran before `setup_logging()`, silently dropping every diagnostic record on the gate path (`Bugs.md` #59); and probes submitted to the `ThreadPoolExecutor` logged with run id `[-]` because a `ContextVar` does not cross a thread-pool boundary without `contextvars.copy_context().run(...)` — the second occurrence of the exact gap `Decisions.md` ADR-013 first recorded in the sandbox track (`Bugs.md` #60). Full suite: 298 collected, 297 passing, the same one pre-existing unrelated Copilot failure.
