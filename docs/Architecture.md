...
The coding brief tells an agent to read `learnings.md` at two points only — once at dispatch, once before finishing — which is enough to carry a finding from an already-finished writer to a still-running reader, but not enough for two same-wave peers to help each other with a problem both hit within the same short window, since neither read point falls inside that window (`Bugs.md` #54, `Research.md` topic 55).
