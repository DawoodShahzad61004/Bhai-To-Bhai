...
│   │   ├── invoker.py       #     Ported from RAG-work, unchanged (ADR-009)
...
│       ├── supervisor.py    #     Routing + the two termination guards (ADR-011)
...
**Sandbox track.** As of 2026-08-05 the full three-level hierarchy **runs end to end** against a local TGI endpoint and terminates cleanly. Getting there produced nine defects (`Bugs.md` #7–#15), four decisions that apply to the orchestrator (ADR-010 through ADR-013), and one result worth more than the rest: the pipeline's most recent complete run finished with `steps=5 | documents=none` — clean exit, `FINISH`, no exception, and **nothing produced**, because the top-level supervisor never routed to the writing team (`Bugs.md` #15). Its architectural value is the mapping it makes concrete, and the places that mapping breaks:
